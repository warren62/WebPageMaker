...
public class PageEditController {
    ...
    public void handleTagSelected(HTMLTagPrototype element, ArrayList<Button> tagButtons) {
        DataManager dm = (DataManager) app.getDataComponent();
        ArrayList<HTMLTagPrototype> list = dm.getTags();
        for(int i = 0; i < list.size(); i++) {
             if(!list.get(i).isLegalParent(element.getTagName())) {
                tagButtons.get(i).setDisable(true);
            }else {
                 tagButtons.get(i).setDisable(false);
             }
        } 
    ...
    public void handleRemoveElementRequest(TreeItem element) {
	if (enabled) {
	    Workspace workspace = (Workspace) app.getWorkspaceComponent();
	    ...
	    TreeView tree = workspace.getHTMLTree();
	    TreeItem selectedItem = (TreeItem) tree.getSelectionModel().getSelectedItem();
	   HTMLTagPrototype clickedItem = (HTMLTagPrototype) selectedItem.valueProperty().getValue();

	   

	    // ADD THE NEW NODE
//            if(newTag.isLegalParent(selectedTag.getTagName())) {
//	    selectedItem.getChildren().remove(element);
            if(clickedItem.isMainParent(clickedItem.getTagName())) {
                 AppMessageDialogSingleton g = AppMessageDialogSingleton.getSingleton();
                 g.show("Web Page Maker", "You can't delete this item!");
            }else {
                DataManager dm = (DataManager) app.getDataComponent();
                deleteTreeItem(dm.getHTMLRoot(), element);
                app.getGUI().getAppFileController().markAsEdited(app.getGUI());
            }
...
//            }
    ...
    public void deleteTreeItem(TreeItem root, TreeItem elementToDelete) {
        if(root.getChildren().remove(elementToDelete)){
            return;
        }
        
        for(TreeItem ti : (ObservableList<TreeItem>)root.getChildren()) {
            deleteTreeItem(ti, elementToDelete);
        }
