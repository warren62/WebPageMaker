...
public class Workspace extends AppWorkspaceComponent {
    ...
    public Workspace(AppTemplate initApp) throws IOException {
        ...
        PrintWriter pwCSS = new PrintWriter(TEMP_CSS_PATH);
        pwCSS.write("");
        pwCSS.close();

        //tagToolbarScrollPane.setStyle("-fx-background-color: rgb(5,20,200);");
        tagToolbar.setStyle("-fx-background-color: rgb(173,216,231);"
                + "-fx-border-color: rgb(200,200,210);");
        app.getGUI().getAppPane().getTop().setStyle("-fx-background-color: rgb(173,216,230);"
                + "-fx-border-color: rgb(200,200,210);");
        //app.getGUI().getAppPane().getTop().setStyle("-fx-border-color: rgb(200,200,200);");
        ...
        htmlTree.getSelectionModel().selectedItemProperty().addListener(e -> {
            ...
            pageEditController.handleTagSelected(selectedTag, tagButtons);
            reloadWorkspace();
        });
        ...
        removeBtn.setGraphic(new ImageView(new Image("file:images/RemoveElement.png")));
        removeBtn.setMaxWidth(BUTTON_TAG_WIDTH);
        removeBtn.setMinWidth(BUTTON_TAG_WIDTH);
        removeBtn.setPrefWidth(BUTTON_TAG_WIDTH);
        removeBtn.getStyleClass().add(CLASS_TAG_BUTTON);
        tagToolbar.getChildren().add(removeBtn);

        removeBtn.setOnAction(e -> {
            TreeItem selectedItem = (TreeItem) htmlTree.getSelectionModel().getSelectedItem();
            deleteMsg = "Are you sure you want to delete " + selectedItem.toString() + "?";
            AppYesNoCancelDialogSingleton d = AppYesNoCancelDialogSingleton.getSingleton();
            d.show("Web Page Maker", deleteMsg);
            if (d.getSelection().equalsIgnoreCase("YES")) {
        ...
        for (HTMLTagPrototype tag : dataManager.getTags()) {
            ...
            System.out.println(clickedTag.toString() + " 99");
            ...
            tagButton.setOnAction(e -> {
                ArrayList<String> legalParents = clickedTag.getLegalParents();
